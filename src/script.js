...
const API_KEY = '&appid=8e6c3de2d9616404d0a4aecf672d1f7d';
	...
	axios
		...
		.then(res => {
			...
			if (weatherName.id >= 200 && weatherName.id <= 232) {
				bgImage.style.backgroundImage = "url('./src/img/thunderstorm-big.jpg')";
				weather.textContent = 'burza';
			} else if (weatherName.id >= 300 && weatherName.id <= 321) {
				bgImage.style.backgroundImage = "url('./src/img/drizzle-big.jpg')";
				weather.textContent = 'mżawka';
			} else if (weatherName.id >= 500 && weatherName.id <= 531) {
				bgImage.style.backgroundImage = "url('./src/img/rain-big.jpg')";
				weather.textContent = 'deszcz';
			} else if (weatherName.id >= 600 && weatherName.id <= 622) {
				bgImage.style.backgroundImage = "url('./src/img/ice-big.jpg')";
				weather.textContent = 'śnieg';
			} else if (weatherName.id === 741) {
				bgImage.style.backgroundImage = "url('./src/img/fog-big.jpg')";
				weather.textContent = 'mgła';
			} else if (weatherName.id === 800) {
				bgImage.style.backgroundImage = "url('./src/img/sun-big.jpg')";
				weather.textContent = 'słonecznie';
			} else if (weatherName.id >= 801 && weatherName.id <= 804) {
				bgImage.style.backgroundImage = "url('./src/img/cloud-big.jpg')";
				weather.textContent = 'zachmurzenie';
			} else {
				bgImage.style.backgroundImage = "url('./src/img/default-big.jpg')";
				weather.textContent = '...';
			}
